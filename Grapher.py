...
def load_data(filename):
    ...
    for i in range(0,5):
        ...
        error_rate_trace = get_array(next(f))
        data.append((name, (error_rate, final_obj_val, trace_obj_val, error_rate_trace)))
    return data
...
def main():
    if len(sys.argv) < 2:
        ...
        f_3 = plt.figure(3)
        names = []
        for (name, (error_rate, final_obj_val, trace_obj_val, error_rate_trace)) in data:
            names.append(name)
            f_0 = plt.figure(0)
            plt.plot(error_rate)
