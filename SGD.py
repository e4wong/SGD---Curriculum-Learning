...
def SGD(training_set, stepsize_constant, lambda_, error_log, validation_set, init_w=None):
    ...
    errors = []
    error_rate_trace = []
    error_rate_trace.append(calc_error_rate(w, validation_set))
...
def curriculum_learning(sort_by, times_to_run, original_data, w_star, lambda_ = None):
    ...
    for i in range(0, times_to_run):
        ...
        validation_set = data[ : len(data)/2]
        training_set = sort_data(sort_by, training_set, w_star)
        (w, (errors, error_rate_trace)) = SGD(training_set, stepsize_constant_var, lambda_, True, validation_set)
    ...
    for i in range(0, len(avg_trace_objective_func_val)):
        avg_trace_objective_func_val[i] = avg_trace_objective_func_val[i]/times_to_run
    return (error_rate, final_objective_func_val, avg_trace_objective_func_val, error_rate_trace)
...
def main():
    if len(sys.argv) < 2:
        ...
        (result, (errors, error_rate_trace)) = run_SGD(training_set, validation_set, stepsize_constant_var, True)
        ...
        (hard_error_rate, hard_objective_func_val, obj_plot_hard, hard_err_trace) = curriculum_learning("hard", num_runs, data, wstar, lambda_)
        print "Running Easy->Hard"
        (easy_error_rate, easy_objective_func_val, obj_plot_easy, easy_err_trace) = curriculum_learning("easy", num_runs, data, wstar, lambda_)
        print "Running Random"
        (random_error_rate, random_objective_func_val, obj_plot_random, random_err_trace) = curriculum_learning("random", num_runs, data, wstar, lambda_)
        print "Running Hard Half"
        (hh_error_rate, hh_objective_func_val, obj_plot_hh, hh_err_trace) = curriculum_learning("hardhalf", num_runs, data, wstar, lambda_)
        print "Running Easy Half"
        (eh_error_rate, eh_objective_func_val, obj_plot_eh, eh_err_trace) = curriculum_learning("easyhalf", num_runs, data, wstar, lambda_)
